...
const setConfig = (config: Partial<ToolbarConfig>): void => {
  ...
  window.dispatchEvent(new CustomEvent("astro-grab-config-change", { detail: updated }));
};

const updateAstroGrab = (config: Partial<ToolbarConfig>): void => {
  window.dispatchEvent(new CustomEvent("astro-grab-config-update", { detail: config }));
};

const toggleAstroGrab = (enabled: boolean): void => {
  window.dispatchEvent(new CustomEvent("astro-grab-toggle", { detail: { enabled } }));
};
...
export default defineToolbarApp({
  init(canvas, app) {
    ...
    headerContainer.style.cssText = "display: flex; justify-content: space-between; align-items: center; padding: 16px 16px 12px;";
    ...
    contentContainer.style.cssText = "padding: 12px 16px 16px; display: flex; flex-direction: column; gap: 16px;";

    const enabledSection = document.createElement("div");
    enabledSection.style.cssText = "display: flex; justify-content: space-between; align-items: center;";
    ...
    hueSection.style.cssText = "display: flex; flex-direction: column; gap: 8px;";
    ...
    hueSlider.style.cssText = "flex: 1; height: 4px; -webkit-appearance: none; appearance: none; background: #e5e7eb; border-radius: 2px; cursor: pointer;";
    ...
    hueValue.style.cssText = "font-size: 12px; min-width: 32px; text-align: right; font-family: monospace;";
    ...
    durationSection.style.cssText = "display: flex; flex-direction: column; gap: 8px;";
    ...
    durationSlider.style.cssText = "flex: 1; height: 4px; -webkit-appearance: none; appearance: none; background: #e5e7eb; border-radius: 2px; cursor: pointer;";
    ...
    durationValue.style.cssText = "font-size: 12px; min-width: 48px; text-align: right; font-family: monospace;";
    ...
    actionsContainer.style.cssText = "display: flex; gap: 8px; padding-top: 8px;";
    ...
    resetButton.addEventListener("click", () => {
      ...
      const statusText = document.getElementById("astro-grab-status-text");
      if (statusDot && statusText) {
        statusDot.style.backgroundColor = DEFAULT_CONFIG.enabled ? "#22c55e" : "#ef4444";
        statusText.textContent = DEFAULT_CONFIG.enabled ? "Enabled" : "Disabled";
      }
